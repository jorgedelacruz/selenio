    ...
    internal class InternalHtmlReporter
    {
        ...
        public static void Initialize(string methodName, string className, string resultsFolder, string assemblyDropDirectory)
        {
            ...
            if (previousTestClassName != className)
            {
                ...
                GetEmbeddedFile(assemblyDropDirectory, "Selenio.Extensions", "Reporting.Style.Summary.xsl", resultsDir + @"\Template\Summary.xsl");
                GetEmbeddedFile(assemblyDropDirectory, "Selenio.Extensions", "Reporting.Style.Details.xsl", resultsDir + @"\Template\Details.xsl");
                GetEmbeddedFile(assemblyDropDirectory, "Selenio.Extensions", "Reporting.Style.dbgstyle.css", resultsDir + @"\Template\dbgstyle.css");
            }
            else
            {
        ...
        public static void Log(string message, bool appendSectionBreak = false)
        {
            string lineEnd = Environment.NewLine + (appendSectionBreak ? Environment.NewLine + "-----------------------" + Environment.NewLine : "");
            File.AppendAllText(resultsDir + @"\log.txt", DateTime.Now.ToString() + ": " + message + lineEnd);
        }
